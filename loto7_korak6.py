#!/usr/bin/env python3
"""Korak 6 — pun pipeline → loto7_korak6.txt"""
...
def main() -> None:
    p = argparse.ArgumentParser(description="Korak 6 — pun pipeline")
    ...
    eng = Loto7Engine(kola, korak=6, seed=args.seed)
    combo, sc = eng.najbolja(preth=poslednje_kolo(kola), max_pokusaja=args.pokusaja)
    out = upisi_predikciju(6, kola, combo, sc, args.csv)
    print(f"Korak 6 → {out}")
...
"""
Loto 7/39 — KORAK 6
Pun pipeline — svi slojevi
Generisano: 23.06.2026 11:15
